...
| `unit_drop_type` | uint | Personality type of the dropped unit. **`0` = random** — the server rolls one per mission start. | **[verified]** `F_UNIT_TYPE_MST`: **1–6** (1=Lord, 2=Anima, 3=Breaker, 4=Guardian, 5=Oracle, 6=Rex). `0` is the recommended default: `MissionArchiver::rollUnitType` weights the roll by that table's own `appearance_rate` column (23/23/22/22/10/0, summing to 100), so replaying a mission can award a different variant and Rex never appears — its rate is 0.00, exactly as in the game. |
...
### Editor defaults for a new enemy

Placing an enemy fills the drop fields with the game's common case rather than
zeroes, so the usual enemy needs no drop editing at all. All of it is one
control away from being changed or cleared.

| Field | Default | Why |
|---|---|---|
| `unit_drop_id` | **the enemy's own `unit_id`** | Nearly every Grand Gaia monster dropped itself; a different unit is the exception. |
| `unit_drop_level` | `1` | Level 0 reaches the client as a broken record (§8 rule). |
| `unit_drop_type` | `0` — random | The server rolls the personality per mission start against `F_UNIT_TYPE_MST`'s own rates, so an author never hardcodes a variant. |
| `unit_drop_chance` | `10` % | |
| `treasure_chest_chance` | `10` % | |
| `treasure_drops` | the **area's whole material table** + a zel and a karma row | From `/api/default-drops` (§7.1), cached per mission so placing an enemy stays instant. The coin rows go in even when the area has no recorded materials, so a chest that can open is never empty. |

**Bosses get the chest but no unit drop** (`unit_drop_id`/`level`/`chance` all
`0`): a boss was the mission's objective, not something farmed off it. This
applies to an enemy placed into a wave already flagged `is_boss`, and to every
enemy in a boss wave built from a template. The capturable bosses that did
exist are the exception an author sets deliberately.
...
## 7.1 Area-scoped material drop tables (recovered from the Global wiki)

Nothing in our MSTs records **which materials dropped where** — that mapping
died with the servers. The Global wiki's per-material `howToObtain` list has it,
so `tools/wiki_item_scrape.py` recovers it and the editor serves it back as
ready-made `TreasureDrop` entries.

**How items are joined to our ids.** The wiki's Item template carries `|id`
(the rendered page hides it), but only 65 of 331 material pages fill it in. The
rest are matched on the **artwork filename**, which both sides store
independently (`item_thum_904.png`), narrowed by sell value and item type —
materials share artwork heavily, so 198 filenames map to more than one item and
the narrowing is what makes the match safe. Result: **260 of 331 resolved (65
declared + 195 by artwork), all 260 agreeing on artwork; 71 stay ambiguous and
are recorded WITH their candidate list rather than guessed at.**

**Why area-level and not per-mission.** Only 1 of 1532 dungeon names and 2 of
3433 mission names resolve to English on our side, so the wiki's dungeon and
mission text cannot be matched to our ids for the bulk of the game. Area names
do resolve (357 of 411 are literal English in F_AREA_MST), so the region is the
join. That is also the granularity a "uniform drop table for this area" wants.
The dungeon/mission strings are still carried per item as `sources`, so an
author can see exactly which fight the wiki attributed a drop to.

**Endpoints:**

| Route | Returns |
|---|---|
| `/api/drop-tables` | every area's table; `?area=<id>` for one |
| `/api/default-drops?mission=<id>` | that mission's area table, already shaped as `TreasureDrop` entries (`target_type` 4) — `&weight=` to set the uniform weight |

Weights are uniform by design: the wiki records *which* materials an area drops,
never their rates, so per-item odds would be invention. A flat table is the
honest starting point and the author tunes it.

Coverage today: **26 areas**, led by St. Lamia (15 materials), Cordelica (14),
Morgan (11), Mistral (9). Regions with no match are either event sources
("Daily", "GQ", "Event Bazaar") or Global-only content absent from our JP MST
(Rokkzalm, Valdroar, Vilanciel, St. Creek) — both are reported, never guessed.

## 7.2 Original monster line-ups (recovered from the Global wiki)

Each wiki dungeon page holds one `{{Quest}}` template per mission carrying the
monsters it fielded (`boss1..boss5`, which are **unit ids** — 60031 is King
Gloomy — so they drop straight into `BattleMonster.unit_id`), plus `energy`,
`battles`, `exp`, per-mission `drops` and capturable units.
`tools/wiki_zone_scrape.py` walks region pages → dungeon pages → quests.

**Joining a wiki quest to our mission id: the (energy, battles, exp)
signature.** Not by name — only 2 of 3433 mission names resolve to English on
our side. Matched within the mission's area, where **90.4% of signatures are
unique**; collisions are reported as `ambiguous` and left unassigned.

*Position matching would be wrong.* The wiki lists quests in display order, not
id order: Adventurer's Prairie runs Guardians / Monsters / Start of Adventure
while our ids run 10 / 11 / 12.

Coverage: **927 quests parsed → 688 mission templates, 659 with monsters**
(74% matched; 96 ambiguous, 126 with no signature match — largely Global-only
content absent from our JP MST).

### Curated rosters — the `{{Zone:Footer}}` find

The `{{Quest}}` templates only name the boss. The **dungeon-level
`{{Zone:Footer}}`** is what carries the regular-monster roster — the
"Monsters:" line on the rendered page — plus that dungeon's own item drops:

    {{Zone:Footer
    |units     =[[Burny]], [[Squirty]], [[Mossy]], [[Sparky]], [[Glowy]],
                [[Gloomy]], [[Goblin]], [[Merman]], [[Mandragora]], [[Harpy]]
    |drops     = [[Green Drop]], [[Blue Drop]], [[Don Nut]], [[Green Grass]]
    |dropsrare = }}

Names resolve to ids against the 2231-unit wiki scrape (same corpus the pages
link to, so they match exactly) — **zero unresolved names across every dungeon
page**. A few pages use `monsters` instead of `units`; both are read.

**Area rosters fill the gap.** The wiki only fills `units` on ~69 dungeon
pages, so an area roster is computed as the union of its dungeons' rosters.
A mission in an unfilled dungeon still gets "monsters seen in this area",
which beats picking from 2291. Coverage: **49 dungeon rosters (524 unit slots)
and 9 area rosters** — Cordelica 56, Lizeria 50, Ryvern 45, St. Lamia 44,
Palmyna 34, Mistral 31, Morgan 30, Encervis 23, Amdahl 8. Of 688 mission
templates, **213 carry a dungeon roster and 280 an area roster**.

The palette's **scope selector** defaults to the narrowest real roster
(dungeon → area → all). A scope picked by hand is kept until you move to
another mission.

**What a template does and does not give you.** `bossN` is the BOSS wave only —
the regular waves were never recorded anywhere and remain the author's job. So
"⤓ Original line-up" appends one boss-flagged wave and says so in the confirm
dialog. It fills `energy_cost`/`exp` only when they are still zero, so it never
overwrites a value you set.

**Wave lay-out on "+ From MST".** Picking a mission from the game data creates
its waves immediately: `battle_count` of them (the MST's own number), with the
last one built from the template when one exists. The regular waves are created
**empty on purpose** — nothing preserved which monsters they held, so filling
them would be invention. What you get is the right *shape*: the correct number
of waves and the boss where the boss belongs, with the validation list naming
each wave still waiting on enemies.

| Route | Returns |
|---|---|
| `/api/mission-template?mission=<id>` | that mission's quest, monsters (with names), drops, energy/battles/exp |
...
**Parked — mimics (`🎁 Mimic` preset removed 2026-08-13).** The editor briefly shipped a preset that
authored a mimic as *an enemy that drops itself*, i.e. a capture chance. That is not what a mimic is.
In the original game a mimic **replaces a treasure chest**: the chest you would have opened becomes a
shaking chest that fights you. The mechanic therefore lives on the *chest*, not on a monster in the
wave line-up — so it needs a field our archive model does not have, and nothing in the captured
`MissionStart` response shows where it would go. Rather than ship an approximation that authors would
have to unlearn, the preset is gone until the binary says how chest→mimic substitution is encoded
(same `tools/ida` readparam workflow as the AI grammar above). **This blocks nothing:** a mission
creator does not need mimics, and the units themselves (60142 Mimic, 60143 Bat Mimic, 60224 Metal
Mimic) are in the palette like any other monster for anyone who wants to place one by hand.
